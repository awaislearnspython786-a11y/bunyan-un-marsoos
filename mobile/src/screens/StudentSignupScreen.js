...
export default function StudentSignupScreen({ navigation }) {
  const [form, setForm] = useState({ name: '', address: '', phone: '', password: '', confirmPassword: '' });
  const [loading, setLoading] = useState(false);
  const [activeInput, setActiveInput] = useState(null);

  const handleSignup = async () => {
    if (!form.name || !form.phone || !form.password) {
      alert('Please fill required fields (Name, Phone, Password)');
      return;
    }
      ...
      >
        <ScrollView contentContainerStyle={styles.container} keyboardShouldPersistTaps="handled">
          <View style={styles.headerContainer}>
            ...
            <Text style={styles.subtitle}>Find your perfect Quran teacher today.</Text>
          </View>
          ...
          <View style={styles.formCard}>
            ...
            <TouchableOpacity style={styles.btn} onPress={handleSignup} disabled={loading}>
              {loading ? <ActivityIndicator color="#FFF" /> : <Text style={styles.btnText}>Register & Search Teachers</Text>}
            </TouchableOpacity>
